...
CCPocket 产品介绍落地页。单文件静态站：全部内容在 `public/index.html`（含一段内联 `<script>`）。
...
```bash
wrangler pages deploy public --project-name=click-bug-demo --branch=main --commit-dirty=true
```
...
2. 修改 `public/index.html`（本站只有这一个页面 + 一段内联脚本）。
3. 读一遍改动，确认逻辑正确。
